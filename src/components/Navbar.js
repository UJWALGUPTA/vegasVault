...
export default function Navbar() {
  ...
  const handleDeposit = async () => {
    ...
    if (amount < TREASURY_CONFIG.LIMITS.MIN_DEPOSIT) {
      notification.error(`Minimum deposit amount is ${TREASURY_CONFIG.LIMITS.MIN_DEPOSIT} STT`);
      return;
    }
    
    if (amount > TREASURY_CONFIG.LIMITS.MAX_DEPOSIT) {
      notification.error(`Maximum deposit amount is ${TREASURY_CONFIG.LIMITS.MAX_DEPOSIT} STT`);
      return;
    }
    ...
    try {
      ...
      if (chainId !== expectedChainId) {
        ...
        try {
          ...
          // If Somnia Network is not added, add it
